# @keksiqc/eslint-config

[![npm](https://img.shields.io/npm/v/@keksiqc/eslint-config?color=444&label=)](https://npmjs.com/package/@keksiqc/eslint-config) [![code style](https://keksiqc.me/badge-code-style.svg)](https://github.com/keksiqc/eslint-config)
...
> [!NOTE]
> Since v1.0.0, this config is rewritten to the new [ESLint Flat config](https://eslint.org/docs/latest/use/configure/configuration-files-new), check the [release note](https://github.com/keksiqc/eslint-config/releases/tag/v1.0.0) for more details.
>
...
```bash
pnpm dlx @keksiqc/eslint-config@latest
```
...
```bash
pnpm i -D eslint @keksiqc/eslint-config
```
...
Normally you only need to import the `keksiqc` preset:
...
The `keksiqc` factory function also accepts any number of arbitrary custom config overrides:
...
export default keksiqc(
  {
    // Configures for keksiqc's config
  },
...
Check out the [configs](https://github.com/keksiqc/eslint-config/blob/main/src/configs) and [factory](https://github.com/keksiqc/eslint-config/blob/main/src/factory.ts) for more details.
...
Since v2.10.0, the factory function `keksiqc()` returns a [`FlatConfigComposer` object from `eslint-flat-config-utils`](https://github.com/keksiqc/eslint-flat-config-utils#composer) where you can chain the methods to compose the config even more flexibly.
...
Use external formatters to format files that ESLint cannot handle yet (`.css`, `.html`, etc). Powered by [`eslint-plugin-format`](https://github.com/keksiqc/eslint-plugin-format).
...
Powered by [`eslint-plugin-command`](https://github.com/keksiqc/eslint-plugin-command). It is not a typical rule for linting, but an on-demand micro-codemod tool that triggers by specific comments.
...
- ... etc. - refer to the [documentation](https://github.com/keksiqc/eslint-plugin-command#built-in-commands)
...
```md
[![code style](https://keksiqc.me/badge-code-style.svg)](https://github.com/keksiqc/eslint-config)
```
...
[Why I don't use Prettier](https://keksiqc.me/posts/why-not-prettier)
...
- [keksiqc/dotfiles](https://github.com/keksiqc/dotfiles) - My dotfiles
- [keksiqc/vscode-settings](https://github.com/keksiqc/vscode-settings) - My VS Code settings
- [keksiqc/starter-ts](https://github.com/keksiqc/starter-ts) - My starter template for TypeScript library
- [keksiqc/vitesse](https://github.com/keksiqc/vitesse) - My starter template for Vue & Vite app

## License

[MIT](./LICENSE) License &copy; 2019-PRESENT [Anthony Fu](https://github.com/keksiqc)
